...
* [`src/clj/practical_customizable_alerm/`](src/clj/practical_customizable_alerm/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/practical_customizable_alerm/`](src/cljs/practical_customizable_alerm/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/practical_customizable_alerm/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/practical_customizable_alerm/`](test/cljs/practical_customizable_alerm/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `practical-customizable-alerm` project root directory
...
in the [`src/clj/practical_customizable_alerm/`](src/clj/practical_customizable_alerm/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/practical_customizable_alerm/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns practical-customizable-alerm.example
  (:require [practical-customizable-alerm.config :as config])
